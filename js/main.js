...
<div class="site-header__topbar">
  <div class="container">
    <div class="quick">
      ...
      <a href="tel:+19172099772">+1 (917) 209-9772</a>
    </div>
    <div class="quick">
...
`;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('form[data-validate]').forEach(form => {
      form.addEventListener('submit', async e => {
        ...
        try {
          ...
          // Network / service unavailable (FormSubmit not activated, CORS,
          // offline, etc.) → fall straight to mailto: so the user's message
          // is never lost. Opens their mail app pre-filled and shows a
          // friendly banner explaining what happened.
          openMailtoFallback(form, payload);
        } finally {
    ...
    function showError(form, payload) {
      ...
      const to = endpoint.split('/').pop() || 'info@asaal.org';
    ...
    // Auto-open the user's email client pre-filled with the message,
    // then show a friendly success-style banner so they know what happened.
    // Used as the failure path for the AJAX endpoint — guarantees the
    // message is never lost even if FormSubmit / network is unavailable.
    function openMailtoFallback(form, payload) {
      const endpoint = form.getAttribute('data-endpoint') || '';
      const to = (endpoint.split('/').pop() || 'info@asaal.org').replace(/^ajax$/, 'info@asaal.org');
      const subject = encodeURIComponent(payload._subject || 'ASAAL website contact form');
      const body = encodeURIComponent(
        'Name: '    + (payload.name    || '') + '\n' +
        'Email: '   + (payload.email   || '') + '\n' +
      ...
      // Trigger the user's mail handler in a way Safari + Firefox accept.
      // Using location.href works inside the user-initiated submit handler.
      try { window.location.href = mailtoUrl; } catch (_) {}

      // Repaint the success banner with a mailto-aware message + manual link.
      const banner = form.querySelector('.form-success');
      const errBox = form.querySelector('.form-error');
      if (errBox) errBox.hidden = true;
      if (banner) {
        banner.innerHTML =
          ...
          'Your message is ready to send from your mail client. ' +
          'If nothing opened, <a href="' + mailtoUrl + '" style="color:inherit; text-decoration:underline;">click here</a> ' +
          'or email <a href="mailto:' + to + '" style="color:inherit; text-decoration:underline;">' + to + '</a> directly.';
        banner.style.display = 'block';
        form.querySelector('.fields')?.style.setProperty('display','none');
        banner.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
      }
